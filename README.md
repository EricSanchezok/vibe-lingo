...
- `@ericsanchezok/synergy-plugin` and `@ericsanchezok/synergy-plugin-kit` `>=3.0.11`; these npm package versions follow the Synergy product release and are independent of `apiVersion: "4.0"`
- Bun `>=1.3.0`
...
bun install
bun run build
synergy plugin add file:///absolute/path/to/vibe-lingo
...
A persistent cache hit returns immediately. A miss asks the private translator for only the translated text and detected source-language tag; VibeLingo derives the destination in code. Common equivalent field names are normalized, and malformed structured output receives one bounded repair attempt before a clean retryable error is shown. Translation, review, presentation, and learning analysis use Synergy's 120-second per-call plugin limit; the language classifier uses 60 seconds per attempt. Synergy's configured model-role fallback and one provider-level retry remain the lower-level availability fallback. Valid output is checked for language direction and size before a short SQLite write. Concurrent identical requests share a single in-process flight. Changing the translation model role does not invalidate cached translations; **Translate again** bypasses the cache and refreshes the record.
...
- the complete normalized selected source (at most 4,000 Unicode code points) and complete validated translation artifact (at most 8,000 Unicode code points) when translation history is enabled;
...
Translation cache identity uses SHA-256 over the normalized selection, language pair, destination policy, and translation-contract version. The hash is only an identity key. When translation history is enabled, the normalized selected source and validated translation are both stored in the local plugin database, regardless of whether the selection is code, a path, a URL, or an entire message. When history is disabled, new translations are not persisted and use a 30-minute process-local cache. The memory cache holds at most 100 entries and is cleared on plugin reload.
...
The complete clean-install release gate is `bun run release:check`. It builds the current artifact before validating packaged metadata, so a stale local `dist` cannot be mistaken for the release candidate.

The built `dist/plugin.json` is generated by plugin-kit and must not be maintained by hand.

## Documentation

- [`docs/architecture.md`](./docs/architecture.md) describes runtime paths,
  layers, persistence, and failure boundaries.
- [`docs/learning-model.md`](./docs/learning-model.md) explains the learning
  principles, evidence lifecycle, review rules, and limitations.
- [`docs/decisions.md`](./docs/decisions.md) records the durable product and
  architecture decisions behind the current implementation.
- [`CONTRIBUTING.md`](./CONTRIBUTING.md) covers development and verification.
- [`SECURITY.md`](./SECURITY.md) explains private vulnerability reporting and
  the privacy invariants expected of changes.
