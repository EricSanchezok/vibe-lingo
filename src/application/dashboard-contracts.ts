...
export const JourneyEventSchema = z.object({
  ...
  correctionBatchId: z.string().uuid().optional(),
...
export const LearningSummaryOutputSchema = z.object({
  ...
  correctionsToday: z.number().int().nonnegative(),
  acceptedFindingsToday: z.number().int().nonnegative(),
  correctionsAnalyzing: z.number().int().nonnegative(),
  correctionsFailed: z.number().int().nonnegative(),
